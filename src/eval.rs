        ...
        match node {
            ...
            Node::Statement(statement) => match statement.clone().kind {
                StatementKind::DrawShape(name, properties) => {
                    ...
                    match shape_kind {
                        ShapeKind::Polygon => {
                            let mut poly_points = Vec::<(f32, f32)>::new();

                            // color
                            let mut poly_config = (u8::from(0), u8::from(0), u8::from(0));
                            ...
                            for i in 0..generations {
                                // push to shapes
                                polyg.update();
                                self.shapes.push(polyg.clone().shape);

                                polyg = polyg.clone();
                                evolve_fn(&mut polyg, statements.clone());
                            }
                        }
